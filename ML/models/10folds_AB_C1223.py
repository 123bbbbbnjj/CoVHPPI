...
output_dir = f"out/preds/10folds_C1223_AB_" + "_".join(info_list)  # 设置输出文件夹
...
for foldn in range(1, 11):
    ...
    model = AdaBoostClassifier(algorithm='SAMME', learning_rate=0.1, n_estimators=50)

    # 保存模型的最佳参数
    with open(f"{output_model_dir}/10folds_C1223_AB_" + "_".join(info_list)+f"_best_params_{foldn}.txt", "w") as f:
    ...
    # save model
    model_file_name = f"{output_model_dir}/10folds_C1223_AB_" + "_".join(info_list)+f"_foldn_{foldn}.pkl"
