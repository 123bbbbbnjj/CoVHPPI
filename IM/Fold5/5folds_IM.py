...
if __name__ == '__main__':
    ...
    for foldn in range(1, 6):
        ...
        vhppi_scores_df = pd.read_csv(f"VHPPI_fold5/VHPPI_scores_fold{foldn}.txt", sep='\t', header=None)
        Scores_df = pd.concat([hppi_scores_df, vhppi_scores_df], ignore_index=True)

        my_im = IM(test_df, Select_pair_file, Scores_df)
        ppi_test, v_set, h_set = my_im.generate_id_groups()
        my_result_dict = my_im.get_scores_dict(ppi_test, v_set, h_set)
        my_result_df = my_im.get_scores_df(my_result_dict)
