...
class DDI:
    ...
    def get_scores_df(self, result_dict):
        ...
        ddi_result_df = pd.DataFrame(list(result_dict.items()), columns=['PPIid', 'Score'])
        ddi_result_df['Score'] = ddi_result_df['Score'].astype(float)
        ddi_result_df[['Vid', 'Hid']] = ddi_result_df['PPIid'].str.rsplit('-', n=1, expand=True)
        ddi_result_df = ddi_result_df[['Vid', 'Hid', 'Score']]
        self.test_df.columns = ['Vid', 'Hid', 'Label']
        result_df = pd.merge(self.test_df, ddi_result_df, on=['Vid', 'Hid'], how='left')
...
if __name__ == '__main__':
    ...
    Select_pair_file = "../hmmscan_result/hmmscan_id_domain.xlsx"
    # 模板评分文件HPPI
    hppi_scores_df = pd.read_csv("../ALLDomain_scores.txt", sep='\t', header=None)
    ...
    for foldn in range(1, 11):
        ...
        # 模板评分文件(HPPI+Train)
        vhppi_scores_df = pd.read_csv(f"VHDDI_C1C2C3/VHDDI_scores_fold{foldn}.txt", sep='\t', header=None)
        ...
        # c1
        print("-----C1-----")
        c1_ddi = DDI(c1_df, Select_pair_file, Scores_df)
        ppi_c1, v_set_c1, h_set_c1 = c1_ddi.generate_id_groups()
        c1_result_dict = c1_ddi.get_scores_dict(ppi_c1, v_set_c1, h_set_c1)
        c1_result_df = c1_ddi.get_scores_df(c1_result_dict)
        # c2h
        print("-----C2h-----")
        c2h_ddi = DDI(c2h_df, Select_pair_file, Scores_df)
        ppi_c2h, v_set_c2h, h_set_c2h = c2h_ddi.generate_id_groups()
        c2h_result_dict = c2h_ddi.get_scores_dict(ppi_c2h, v_set_c2h, h_set_c2h)
        c2h_result_df = c2h_ddi.get_scores_df(c2h_result_dict)
        # c2v
        print("-----C2v-----")
        c2v_ddi = DDI(c2v_df, Select_pair_file, Scores_df)
        ppi_c2v, v_set_c2v, h_set_c2v = c2v_ddi.generate_id_groups()
        c2v_result_dict = c2v_ddi.get_scores_dict(ppi_c2v, v_set_c2v, h_set_c2v)
        c2v_result_df = c2v_ddi.get_scores_df(c2v_result_dict)
        # c3
        print("-----C3-----")
        c3_ddi = DDI(c3_df, Select_pair_file, Scores_df)
        ppi_c3, v_set_c3, h_set_c3 = c3_ddi.generate_id_groups()
        c3_result_dict = c3_ddi.get_scores_dict(ppi_c3, v_set_c3, h_set_c3)
        c3_result_df = c3_ddi.get_scores_df(c3_result_dict)
